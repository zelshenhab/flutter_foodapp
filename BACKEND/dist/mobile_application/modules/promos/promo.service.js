...
exports.recordPromoUsage = recordPromoUsage;
...
async function validatePromo(code, subtotal, userId) {
    ...
    if (userId) {
        const { data: existingUsage } = await supabase_1.supabase
            .from("PromoUsage")
            .select("*")
            .eq("promoId", promo.id)
            .eq("userId", userId)
...
async function recordPromoUsage(promoId, userId, orderId, discountAmount) {
    ...
    await supabase_1.supabase
        .from("Promo")
        .update({ usedCount: supabase_1.supabase.rpc('increment', { row_id: promoId }) })
        .eq("id", promoId);
}
